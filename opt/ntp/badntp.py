...
import pathlib
#Define variables
ntpserver1="ntp server x.x.x.x prefer"
ntpserver2="ntp server x.x.x.x"
ntpserver3="ntp server vrf Mgmt-intf x.x.x.x prefer"
ntpserver4="ntp server vrf Mgmt-intf x.x.x.x"
...
# If statment will ignore  Core Switches since these are NTP servers
    if host.endswith("switch1") or host.endswith("switch2"):
      p=0
    else:
#NTP Server
      found = False
